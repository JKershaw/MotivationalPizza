var AllByUserId = require("./queries/AllByUserId"),
	...
	AllTagsByUserId = require("./queries/AllTagsByUserId"),
	PopularTagsByUserId = require("./queries/PopularTagsByUserId");

var TodayFullSpecification = require("./specifications/TodayFullSpecification");

module.exports = function (eventStore, usersRepository) {
	...
	function doneBeforeTodayByUserId(userId, callback) {
		new DoneBeforeTodayByUserId(tasksRepository).doneTodayByUserId(userId, callback);
	}
	...
	function allTagsByUserId(userId, callback) {
		new AllTagsByUserId(tasksRepository).allTagsByUserId(userId, callback);
	}

	function popularTagsByUserId(userId, numberOfResultsWanted, callback) {
		new PopularTagsByUserId(tasksRepository).popularTagsByUserId(userId, numberOfResultsWanted, callback);
	}

	function isTodayFull(userId, callback) {
		new TodayFullSpecification(tasksRepository, usersRepository).isSatisfiedBy(userId, callback);
	}

	function latestEvent(callback) {
		eventStore.latestEvent(callback);
	}
